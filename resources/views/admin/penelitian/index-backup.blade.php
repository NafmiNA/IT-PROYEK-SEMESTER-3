...
<x-app-layout backUrl="{{ route('admin.dashboard') }}">
    <div class="max-w-7xl mx-auto px-6 py-8 space-y-6">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <p class="text-xs uppercase tracking-wider text-[#2050A0]/70">Manajemen Penelitian (Admin)</p>
                <h2 class="text-2xl font-semibold text-[#2050A0]">Kelola Penelitian (Global)</h2>
                <p class="text-sm text-gray-500">Pantau seluruh proposal dan aktivitas penelitian semua dosen.</p>
            </div>
        ...
        <section class="overflow-hidden rounded-3xl border-2 border-gray-200 bg-white ring-1 ring-gray-200/70 shadow-lg">
            <header class="flex items-center justify-between border-b border-gray-100 bg-gradient-to-r from-[#2050A0]/10 to-[#2050A0]/5 px-6 py-4">
                ...
                @endphp
                <div>
                    <h3 class="text-lg font-semibold text-[#2050A0]">Daftar Penelitian</h3>
                    <p class="text-xs text-gray-500">Total {{ $totalPenelitian }} penelitian tercatat (Global)</p>
                </div>
            </header>
                            ...
                            <tr class="transition hover:bg-[#2050A0]/5">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2 text-xs font-semibold">
                                        {{-- MODIFIKASI: Rute diubah ke admin.penelitian.show --}}
                                        <a href="{{ route('admin.penelitian.show', $p) }}" class="inline-flex items-center gap-2 rounded-full border border-[#2050A0]/30 px-3 py-1 text-[#2050A0] transition hover:bg-[#2050A0] hover:text-white">
                                        ...
                                        {{-- MODIFIKASI: Rute diubah ke admin.penelitian.edit --}}
                                        <a href="{{ route('admin.penelitian.edit', $p) }}" class="inline-flex items-center gap-2 rounded-full border border-amber-300 px-3 py-1 text-amber-600 transition hover:bg-amber-500 hover:text-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        ...
                                        {{-- MODIFIKASI: Rute diubah ke admin.penelitian.destroy --}}
                                        <form action="{{ route('admin.penelitian.destroy', $p) }}" method="POST" onsubmit="return confirm('Hapus penelitian ini?');">
                                            @csrf
                                            @method('DELETE')
            ...
            @if (method_exists($penelitian, 'links'))
                <footer class="flex items-center justify-between border-t border-gray-100 bg-gray-50 px-6 py-3 text-xs text-gray-500">
                    <span>Menampilkan {{ $penelitian->firstItem() ?? 0 }}—{{ $penelitian->lastItem() ?? 0 }} dari {{ $penelitian->total() ?? $penelitian->count() }} entri</span>
                    {{ $penelitian->links() }}
                </footer>
            @endif
        </section>
