        ...
        <div class="mx-auto max-w-5xl px-4 py-8">
            ...
            <nav class="mb-6 animate-fade">
                <ol class="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <li><a href="{{ route('admin.penelitian.index') }}" class="hover:text-blue-600 transition-colors">Penelitian</a></li>
                    <li><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></li>
                    <li class="font-semibold text-blue-600">Detail Penelitian</li>
                </ol>
            </nav>
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-2">
                        <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider 
                            {{ $penelitian->status === 'Disetujui' ? 'bg-green-100 text-green-700' : 
                              ($penelitian->status === 'Ditolak' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') }}">
                            {{ $penelitian->status }}
                        </span>
                        <span class="text-sm text-gray-500 font-medium">{{ $penelitian->tahun }}</span>
                    </div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 leading-tight">{{ $penelitian->judul }}</h1>
                </div>
                <div class="flex items-center gap-3 flex-shrink-0">
                    <a href="{{ route('admin.penelitian.index') }}" class="px-4 py-2 rounded-lg border-2 border-gray-200 text-gray-600 font-semibold hover:bg-gray-50 transition-colors">
                        Kembali
                    </a>
                    <a href="{{ route('admin.penelitian.edit', $penelitian) }}" class="px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 shadow-md transition-colors flex items-center gap-2">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <section class="rounded-2xl bg-white shadow-lg ring-1 ring-gray-200 overflow-hidden">
                        <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 flex items-center gap-3">
                            ...
                            <h2 class="text-lg font-bold text-white">Informasi Penelitian</h2>
                        </div>
                        <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Skema</p>
                                <p class="text-gray-900 font-medium">{{ $penelitian->skema ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Sumber Dana</p>
                                <p class="text-gray-900 font-medium">{{ $penelitian->sumber_dana ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Jumlah Dana</p>
                                <p class="text-emerald-600 font-bold font-mono text-lg">Rp {{ number_format($penelitian->dana, 0, ',', '.') }}</p>
                            </div>
                            <div>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Link Jurnal</p>
                                @if($penelitian->link_jurnal)
                                    <a href="{{ $penelitian->link_jurnal }}" target="_blank" class="text-blue-600 hover:underline break-all text-sm flex items-center gap-1">
                                        Kunjungi Link
                    ...
                    <section class="rounded-2xl bg-white shadow-lg ring-1 ring-gray-200 overflow-hidden">
                        <div class="bg-gradient-to-r from-amber-500 to-amber-600 px-6 py-4 flex items-center gap-3">
                            ...
                            <h2 class="text-lg font-bold text-white">Laporan & Dokumentasi</h2>
                        </div>
                        <div class="p-6">
                            <div class="mb-6">
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Laporan Akhir</p>
                                @if($penelitian->laporan_path)
                                    <a href="{{ Storage::url($penelitian->laporan_path) }}" target="_blank" class="inline-flex items-center gap-3 p-3 rounded-xl border border-gray-200 bg-gray-50 hover:bg-blue-50 hover:border-blue-200 transition-all group w-full sm:w-auto">
                                        <div class="p-2 bg-red-100 rounded-lg text-red-600 group-hover:scale-110 transition-transform">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                                        </div>
                                        <div class="text-left">
                                            <p class="text-sm font-semibold text-gray-900 group-hover:text-blue-700">Download Laporan.pdf</p>
                                            <p class="text-xs text-gray-500">Klik untuk mengunduh</p>
                                        </div>
                                    </a>
                                @else
                                    <p class="text-sm text-gray-500 italic bg-gray-50 p-3 rounded-lg border border-dashed border-gray-300">Belum ada laporan diunggah.</p>
                                @endif
                            </div>

                            <div>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Galeri Dokumentasi</p>
                                @if($penelitian->dokumentasi && $penelitian->dokumentasi->count() > 0)
                                ...
                                @else
                                    <p class="text-sm text-gray-500 italic">Tidak ada dokumentasi tambahan.</p>
                                @endif
                            </div>
                        </div>
                    </section>
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <section class="rounded-2xl bg-white shadow-lg ring-1 ring-gray-200 overflow-hidden sticky top-6">
                        <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 px-6 py-4 flex items-center gap-3">
                            ...
                            <h2 class="text-lg font-bold text-white">Tim Pelaksana</h2>
                        </div>
                        <div class="p-6 space-y-6">
                            {{-- Ketua --}}
                            <div>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Ketua Peneliti</p>
                                <div class="flex items-center gap-3 p-3 rounded-xl bg-blue-50 border border-blue-100">
                                    <div class="w-10 h-10 rounded-full bg-blue-200 text-blue-700 flex items-center justify-center font-bold text-lg">
                                        {{ substr($penelitian->ketua->nama ?? '?', 0, 1) }}
                                    </div>
                                    <div>
                                        <p class="font-bold text-gray-900 text-sm">{{ $penelitian->ketua->nama ?? '-' }}</p>
                                        <p class="text-xs text-blue-600">{{ $penelitian->ketua->email ?? '' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                @if($penelitian->dosens->where('id', '!=', $penelitian->dosen_id)->count() > 0)
                                    <div class="space-y-2">
                                        @foreach($penelitian->dosens->where('id', '!=', $penelitian->dosen_id) as $dosen)
                            ...
                            <div>
                                ...
                                @if($penelitian->mahasiswas && $penelitian->mahasiswas->count() > 0)
                                    <div class="space-y-2">
                                        @foreach($penelitian->mahasiswas as $mhs)
                                            <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
