...
<div x-data="{ sidebarOpen: localStorage.getItem('sidebarOpen') !== 'false' }" 
    ...
    {{-- Sidebar --}}
    <aside class="fixed top-0 left-0 z-50 h-screen sidebar-solid text-white shadow-2xl transition-all duration-300 flex flex-col"
           :class="[
               sidebarOpen ? 'w-64' : 'lg:w-20 -translate-x-full lg:translate-x-0',
               'lg:translate-x-0'
           ]">
        ...
        <nav class="flex-1 overflow-y-auto py-5 space-y-1 transition-all duration-300"
            ...
            @if(auth()->user()->role !== 'mahasiswa')
                ...
                {{-- Penelitian (Smart based on role) --}}
                @php
                ...
                {{-- Pengabdian (Smart based on role) --}}
                @php
            ...
            @if(auth()->user()->role === 'mahasiswa')
                ...
                <div x-show="sidebarOpen" class="px-3 mb-2">
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Menu Mahasiswa</p>
                </div>

                {{-- Dokumentasi --}}
                <a href="{{ route('mahasiswa.dokumentasi.index') }}" 
                   class="group flex w-full items-center py-2.5 rounded-lg transition-all duration-200 {{ request()->routeIs('mahasiswa.dokumentasi.*') ? 'bg-emerald-600 text-white shadow-lg' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}"
                   :class="sidebarOpen ? 'gap-3 px-3 justify-start' : 'gap-0 px-0 justify-center'">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    <span x-show="sidebarOpen" class="font-medium">Dokumentasi</span>
                </a>
            @endif
